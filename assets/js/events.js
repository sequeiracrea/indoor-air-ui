...
function getPointColor(p){
  ...
  if(score>0.75) return "rgba(255,0,0,";       // rouge instable
  else if(score>0.5) return "rgba(255,165,0,"; // orange alerte
  return "rgba(0,200,0,";                        // vert stable
}
...
function drawPoints(){
  ...
  for(let i=0;i<displayedPoints.length;i++){
    ...
    ctx.fillStyle = getPointColor(p) + (0.3 + 0.7*ageFactor) + ")";
    ctx.fill();
  }
}
...
async function init(){
  ...
  legend.innerHTML = `
    ...
    Points récents plus gros et plus opaques
  `;
