...
/* -------------------------------------------------------
   SCATTER + HISTOGRAMMES AVEC COULEURS INTELLIGENTES
---------------------------------------------------------*/
...
async function loadScatterFromQuery(){
  ...
  /* --- Scatter avec couleurs progressives + densité --- */
  const smartPoints = points.map((p,i)=>{
    const t = i / points.length; // ancien → récent
    const density = localDensity(points, i); // densité locale
    const ratio = Math.min(1, density/10); // plus de densité = plus mélange
    const color = mixColors(GAS_COLORS[xVar], GAS_COLORS[yVar], 0.7*(1-ratio) + 0.3*ratio);
    const bright = Math.min(60, density*4);
    return {
      x: p.x,
      y: p.y,
      backgroundColor: lighten(color, bright),
      borderColor: lighten(color, bright),
      borderWidth:0.6,
      opacity:0.3+t*0.7
    };
  });
...
window.addEventListener("load", async()=>{
  ...
  document.getElementById("btn-update-scatter")
