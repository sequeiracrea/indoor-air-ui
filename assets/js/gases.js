...
/* -------------------------------------------------------
   SCATTER + HISTOGRAMMES AVANCE + LEGEND + CURSEUR
---------------------------------------------------------*/
...
async function loadScatterFromQuery() {
  ...
  // Normalisation pour mix couleur
  ...
  const smartPoints = points.map((p, i) => {
    ...
    const mixedColor = mixColors(GAS_COLORS[xVar], GAS_COLORS[yVar], (ratioX + ratioY)/2);
    const density = localDensity(points, i);
    const bright = Math.min(60, density*5);
    return {
      x: p.x,
      y: p.y,
      backgroundColor: lighten(mixedColor, bright),
      borderColor: lighten(mixedColor, bright),
      borderWidth: 0.6,
      timestamp: p.t
    };
  });
  ...
  // Scatter chart
  const ctxScatter = document.getElementById("gasesScatter").getContext("2d");
  scatterChart = new Chart(ctxScatter, {
    ...
    options: {
      ...
      plugins:{
        tooltip:{
          callbacks:{
            label: item => `${xVar}: ${item.raw.x}, ${yVar}: ${item.raw.y}, t: ${item.raw.timestamp}`
          }
        },
        legend: {
          display:true,
      ...
      onHover: (event, elements, chart) => {
        ...
        document.getElementById("scatterDetails").innerHTML = `
          <strong>Survol :</strong> X=${xVal.toFixed(2)}, Y=${yVal.toFixed(2)}
        `;
      }
    }
  });
