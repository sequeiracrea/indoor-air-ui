...
async function loadScatterFromQuery() {
  ...
  // Met à jour les champs de sélection
  ...
  // Histogrammes : labels vides + titre via légende
  const bins = 20;
  ...
  histXChart = new Chart(document.getElementById("histX"), {
    ...
    options: {
      ...
      scales: { x: { title: { display: true, text: xVar.toUpperCase() } } }
    }
  });
  ...
  histYChart = new Chart(document.getElementById("histY"), {
    ...
    options: {
      ...
      maintainAspectRatio: false,
      plugins: { legend: { display: true } },
      scales: { y: { title: { display: true, text: yVar.toUpperCase() } } }
    }
  });
